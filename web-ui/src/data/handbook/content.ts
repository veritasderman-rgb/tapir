...
export const HANDBOOK: Chapter[] = [
  // ════════════════════════════ ÚVOD ════════════════════════════
  {
    ...
    content: {
      ...
      vs: [
        { t: 'p', html: 'Tato úroveň cílí na <strong>kvantitativní porozumění</strong>: koncepty zavádíme formálně, u klíčových tvrzení odkazujeme na primární literaturu (boxy „Pro hlubší studium" s DOI) a explicitně zmiňujeme předpoklady a jejich meze. Předpokládá se základní znalost diferenciálního počtu a pravděpodobnosti.' },
        { t: 'h', text: 'Co je matematická epidemiologie' },
        { t: 'p', html: 'Matematická epidemiologie popisuje přenos infekce dynamickými systémy (ODR, stochastické procesy, síťové modely) s cílem (i) <em>porozumět</em> mechanismu, (ii) <em>odhadnout</em> nepozorovatelné veličiny (R₀, IFR, generační interval) z pozorovaných dat a (iii) <em>porovnat</em> kontrafaktuální scénáře intervencí. Od popisné epidemiologie (osoba–místo–čas) se liší explicitním generativním modelem přenosu.' },
        { t: 'h', text: 'Notace a klíčové veličiny' },
        { t: 'list', items: [
          '<strong>R₀</strong> — bazální reprodukční číslo (dominantní vlastní číslo NGM)',
          '<strong>R<sub>t</sub> / R<sub>eff</sub></strong> — časově proměnné/efektivní reprodukční číslo',
          '<strong>r</strong> — Malthusovský růstový parametr rané fáze',
          '<strong>T<sub>g</sub></strong> — generační interval (latentní), <strong>T<sub>s</sub></strong> — sériový interval (pozorovatelný)',
          '<strong>β, σ, γ</strong> — rychlosti přenosu, progrese E→I a zotavení I→R',
          '<strong>IFR / CFR</strong> — infekční / case fatality ratio',
          '<strong>k</strong> — disperzní parametr rozdělení sekundárních případů',
        ] },
        { t: 'warn', title: 'Meze modelů', html: 'Kompartmentové modely předpokládají dobře promíchanou populaci a exponenciálně rozdělené doby setrvání — obojí je idealizace. Pro reálné odhady je nezbytná <strong>kalibrace</strong>, <strong>kvantifikace nejistoty</strong> (intervaly, citlivostní analýza) a <strong>validace</strong> out-of-sample. Model bez nejistoty není predikce, ale ilustrace.' },
      ],
    },
  },
  ...
  // ══════════════════════ JAK SE ŠÍŘÍ VIRUS ══════════════════════
  {
    ...
    content: {
      ...
      vs: [
        { t: 'p', html: 'Dynamiku přenosu určuje souhra <strong>infekčnosti</strong>, <strong>kontaktní struktury</strong> a <strong>časování</strong>. Infekčnost lze rozložit do <em>profilu infekčnosti</em> β(τ) — očekávané míry přenosu v čase τ od nákazy. Bazální reprodukční číslo je jeho integrál: R₀ = ∫₀^∞ β(τ) dτ.' },
        { t: 'h', text: 'Klíčové intervaly' },
        { t: 'list', items: [
          '<strong>Latentní doba</strong> — od nákazy po počátek infekčnosti; <strong>inkubační doba</strong> — od nákazy po příznaky. Nemusí se krýt (presymptomatický přenos = infekčnost před příznaky).',
          '<strong>Generační interval T<sub>g</sub></strong> — doba mezi nákazou nakažujícího a jím nakaženého (nepozorovatelný přímo).',
          '<strong>Sériový interval T<sub>s</sub></strong> — doba mezi nástupem příznaků nakažujícího a nakaženého (pozorovatelný; při presymptomatickém přenosu může být i záporný).',
        ] },
        { t: 'info', title: 'Vztah r ↔ R přes generační interval', html: 'Z pozorovaného růstu r nelze odvodit R bez znalosti rozdělení generačního intervalu g(τ): platí <strong>1/R = ∫ e^(−rτ) g(τ) dτ</strong> (Lotkova–Eulerova rovnice). Pro fixní střední T<sub>g</sub> dává předpoklad „vše rovno průměru" horní mez R; realističtější (rozptýlené) rozdělení dává nižší R. Proto je odhad R z r citlivý na předpoklad o g(τ).' },
        { t: 'refs', items: [REF.wallinga2007] },
        { t: 'h', text: 'Heterogenita a superspreading' },
        { t: 'p', html: 'R₀ je <em>průměr</em> individuální reprodukce ν, jejíž rozdělení bývá silně <strong>přerozptýlené (overdispersed)</strong> — modelováno negativně binomickým rozdělením s disperzním parametrem <strong>k</strong> (malé k = silný superspreading; pro SARS k ≈ 0,16). Důsledky: (i) větší pravděpodobnost <em>stochastického vymření</em> zavlečení, (ii) <em>explozivnější</em>, ale vzácnější ohniska, (iii) <strong>cílená</strong> opatření (na superspreading prostředí/události) dominují plošným.' },
        { t: 'info', title: 'Sekundární attack rate (SAR)', html: 'Podíl vnímavých kontaktů, kteří onemocní po expozici indexovému případu v daném prostředí. Empirická míra infekčnosti; klíčový rozdíl domácnost (vysoký SAR) vs. veřejný prostor. Pozor na cenzorování a ko-primární případy.' },
  ...
  // ════════════════════ KRIZOVÉ ŘÍZENÍ ════════════════════
  {
    ...
    content: {
      ...
      vs: [
        { t: 'p', html: 'Krizové rozhodování lze formalizovat jako rozhodování za nejistoty: volíme akci a maximalizující očekávaný užitek E[U(a, θ)] přes posteriorní rozdělení parametrů θ. Užitek je <strong>multikriteriální</strong> (zdraví, ekonomika, společnost); jeho agregace (např. přes QALY/DALY a peněžní ekvivalenty) je sama o sobě hodnotová volba.' },
        { t: 'list', items: [
          '<strong>Hodnota informace (VOI)</strong> — očekávaný přínos snížení nejistoty kvantifikuje, kdy se vyplatí čekat/měřit vs. jednat. Při exponenciálním růstu cena čekání rychle roste.',
          '<strong>Robustnost</strong> — preferuj rozhodnutí odolná vůči chybě v θ (minimax-regret, robustní optimalizace), ne optimální pro jediný bodový odhad.',
          '<strong>Adaptivní řízení</strong> — rozhodovat sekvenčně, s pravidly reagujícími na ukazatele (R<sub>t</sub>, obsazenost JIP) — blíž stop-and-go než jednorázové volbě.',
        ] },
        { t: 'h', text: 'Alokace omezených zdrojů — příklad vakcín' },
        { t: 'p', html: 'Priorizace očkování je explicitní optimalizační problém s odlišnými cíli. Modelová analýza ukázala, že strategie <strong>závisí na cíli</strong>: minimalizace <em>počtu nákaz</em> favorizuje očkování dospělých 20–49 let (hlavní přenašeči), zatímco minimalizace <em>úmrtí a ztracených let života</em> ve většině scénářů favorizuje seniory 60+. Sérologické cílení (přeskočení séropozitivních) zvyšuje marginální přínos dávky a může snižovat nerovnosti.' },
        { t: 'refs', items: [REF.bubar2021] },
        { t: 'warn', title: 'Modely jako podpora, ne náhrada', html: 'Modely kvantifikují <em>kompromisy</em>, ale samotnou volbu vah mezi životy, ekonomikou a svobodami nedávají — ta je politická a etická. Transparentnost předpokladů a nejistoty je proto součástí odpovědného použití modelu v rozhodování.' },
      ],
    },
  },
  ...
  // ════════════════════ PŘÍBĚH ════════════════════
  {
    ...
    content: {
      ...
      vs: [
        { t: 'p', html: 'Případová viněta Dr. Kovářové ilustruje standardní <strong>kroky vyšetřování ohniska</strong> (outbreak investigation), jak je kodifikuje terénní epidemiologie (např. EIS/FETP):' },
        { t: 'list', items: [
